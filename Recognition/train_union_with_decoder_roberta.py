from union.mix_union_vietocr import UnionVietOCR, UnionRoBerta, UnionRoBerta
...
config = Cfg.load_config_from_file("./config/uniroberta_2.yml")
vocab = config.vocab
...
# tokenizer = AutoTokenizer.from_pretrained("KoichiYasuoka/roberta-small-japanese-aozora-char")
# checkpoint = torch.load("/home1/vudinh/NomNaOCR/icocr/union/ckpt_320x48_v3_2/best_decoder.pth", map_location=torch.device("cuda"))
...
train_dataloader = DataLoader(train_dataset, batch_size=config.batch_size, shuffle=config.shuffle, num_workers=16, collate_fn = collate_fn)
...
# model.decoder.layers.requires_grad_(False)
