...
#        output = rearrange(output, 't n e -> n t e')
    ...
    def get_memory(self, memory, i):
        memory = memory[:, [i], :]
        return memory
class LanguageTransformerCustom(nn.Module):
    def __init__(self, vocab_size, 
                 d_model, nhead, 
                 num_encoder_layers, num_decoder_layers, 
                 dim_feedforward, max_seq_length, 
...
#        self.learned_pos_enc = LearnedPositionalEncoding(d_model, pos_dropout, max_seq_length)
        ...
        # self.encoder_layer = nn.TransformerEncoderLayer(d_model, nhead, dim_feedforward, trans_dropout)
