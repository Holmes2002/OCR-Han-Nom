...
class ICOCREncoderGRPC:
    '''
        Sample model-request triton-inference-server with gRPC
    '''
    def __init__(self,
                triton_host = '10.9.3.239:2913', # default gRPC port
                triton_model_name = 'encoder',
                verbose = False,
                ssl = False,
    ...
    def run(self,
        ...
        meta_outputs = [('output_encoder', 'FP32')]):
...
class ICOCRDecoderGRPC:
    '''
        Sample model-request triton-inference-server with gRPC
    '''
    def __init__(self,
                triton_host = '10.9.3.239:2913', # default gRPC port
                triton_model_name = 'decoder',
                verbose = False,
                ssl = False,
                root_certificates = None,
                private_key = None,
    ...
    def run(self,
        data,
        triton_model_name=None,
        meta_inputs = [('tgt_inp', 'INT32'), ('encoder_hidden_states', 'FP32')],
        meta_outputs = [('output_decoder', 'INT32')]):
        if self.triton_model_name is None:
            assert triton_model_name is not None, "Current TritonModelGRPC not config model name, please init connection with triton_model_name or specific when call 'run' "
            current_model = triton_model_name
        else:
